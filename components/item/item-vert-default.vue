<template>
    <div class="item_wrapper">
        <div class="item-list vert" v-for="(item, index) in itemInfo" :key="index">
            <div class="item_thumbnail">
                <div class="user-action">
                    <div class="user-attention user-ico" @click.prevent="addToWishList($event)">관심상품</div>
                    <div v-if="!item.thumbnailArea.date" class="user-basket user-ico" @click="addToWishList($event)">장바구니
                    </div>
                ...
                <div class="ranking" v-if="item.rank"><span class="numbering">{{ fillZero(index + 1) }}</span></div>
